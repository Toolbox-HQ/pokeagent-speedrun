...
class AgentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tensor = decoder.get_frames_at(indices = indices)
...
def infer_labels(frames, idm, device):
    ...
    while idx + 128 <= frames.shape[0]:
        ...
        dummy = {
            ...
            "state_in": idm.initial_state(x.shape[0]).to(device),
        }
...
def load_model(chkpt=".cache/pokeagent/idm.ckpt") -> torch.nn.Module:
    model = IDModel()
...
def save_clip_with_actions(frames, labels, out_path, fps):
    ...
    for f, l in zip(frames, labels):
        img = to_pil_image(f.cpu())  # RGB
        draw = ImageDraw.Draw(img)
        text = CLASS_TO_KEY.get(int(l), str(int(l)))
        draw.text((8, 8), text, fill=(255, 255, 255))
        annotated.append(pil_to_tensor(img).permute(1, 2, 0))  # (H, W, C) uint8
    video = torch.stack(annotated, dim=0)  # (T, H, W, C) uint8
    write_video(out_path, video, fps=int(fps))
...
def main():
    ...
    with open(".cache/intervals.json", "r", encoding="utf-8") as f:
        items = json.load(f)
